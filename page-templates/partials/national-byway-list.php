	...
	if ( ! empty( $nb_query->have_posts() ) ) :
		...
		while ( $nb_query->have_posts() ) :
			...
			if ( $tax === 'aar' ) :
				...
				<li class="byway-item">
					<a href="<?php echo esc_url( $permalink ); ?>" class="item-link"><?php echo esc_html( $query_id ); ?></a>
										<?php
										echo esc_html( $asterisk );
										?>
				</li>
			...
			?>
			<li class="byway-item">
				Currently there are no All-American Roads or National Scenic Byways in <?php echo esc_html( $title ); ?>.
			</li>
			...
			if ( $title === 'Texas' ) :
				...
				<div class="unlinked">
					<p>Currently there are no additional byways in <?php echo esc_html( $title ); ?></p>
				</div>
