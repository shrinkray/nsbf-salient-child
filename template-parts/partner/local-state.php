<?php
/**
 * State-specific local partners template part
 *
 * @package NSBF_Theme
 */
...
// Get state-specific partner fields
$state_partner_heading = get_field( 'sb_state_partner_heading' ) ?: 'State Byway Partners';
?>
    ...
    <div class="section">
        <div class="detail-subsection">
            <?php if ( have_rows( 'sb_local_partner_organization' ) ) : ?>
                <div class="label-minor-heading"><?php echo esc_html( $state_partner_heading ); ?></div>
                <ul class="grid sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4">
                    ...
                    while ( have_rows( 'sb_local_partner_organization' ) ) :
                        ...
                        $partner_state_role = get_sub_field( 'sb_po_state_role' ); // Additional state-specific field
                        ?>
                        
                        <li class="item">
                            <div class="detail-organization">
                                <?php echo esc_html( $partner_organization ); ?>
                                <?php if ( ! empty( $partner_state_role ) ) : ?>
                                    <div class="text-sm text-gray-600">
                                        <?php echo esc_html( $partner_state_role ); ?>
                                    </div>
                                <?php endif; ?>
                            </div>
            ...
            <?php else : ?>
                <div class="text-sm truncate text-mangotango">
                    <?php esc_html_e( 'No state partners available', 'nsbf-theme' ); ?>
                </div>
            <?php endif; ?>
        </div>
    </div>
